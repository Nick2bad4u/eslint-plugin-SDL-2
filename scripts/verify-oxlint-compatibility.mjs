...
const fixturePath = path.join(temporaryRoot, "fixture.tsx");
 ...
 * @param {string} output
 *
 * @returns {readonly NormalizedDiagnostic[]}
 */
...
const main = async () => {
    assert.equal(namespace, "sdl");
    assert.equal(ruleNames.length, 71);
    assert.equal(configNames.length, 9);
    ...
    assert.deepEqual(
        [
            ...new Set(eslintDiagnostics.map(({ ruleName }) => ruleName)),
        ].toSorted(),
        expectedDiagnosticRules
    );
    ...
    const eslintFixedOutput = await readFile(fixturePath, "utf8");
    await writeFile(fixturePath, fixture, "utf8");
    runCli(oxlintCliPath, [
        "-A",
        "correctness",
    ...
    const oxlintFixedOutput = await readFile(fixturePath, "utf8");
    assert.equal(oxlintFixedOutput, eslintFixedOutput);
        ...
        JSON.stringify(
            {
                ...
                diagnosticRules: expectedDiagnosticRules,
                fixedOutputMatches: true,
                namespace,
